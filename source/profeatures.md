...
| <br /> | [Web应用](WebApp.md) 和 <br>[本地化应用](LocalApp.md) | [Excel加载项](ExcelAddIn.md) 和 <br> [本地化 Excel 加载项](LocalExcelAddin.md) | [WPS加载项](WPSAddIn.md) | [API](API.md) | [MCP](MCP.md) |
| :----: | :----------------: | :-----------------------: | :-------------------: | :-----------: | :-----------: |
|   适用   |          ✅         |             ✅             |           ✅           |       ❌       |       ❌       |
...
- 本地化应用`在本地化应用上加载Excel到JSON`
  ...
  - 您可以从Excel、Google表格或任何其他兼容Excel的软件中复制和粘贴Excel数据，数据以制表符分隔
  - 您也可以复制和粘贴逗号分隔的CSV数据
- Excel加载项`在Excel中侧载Excel到JSON`：
  - 使用鼠标或者键盘直接从Excel工作表中选择数据。
  - 或者，让Excel转JSON转换Excel中所有可见工作表。
  - 或者，让Excel转JSON转换Excel中的所有工作表。
- WPS加载项`在WPS中侧载Excel到JSON`：
  - 使用鼠标或者键盘直接从WPS工作表中选择数据。
...
| id | <mark>student\_name</mark> | student\_familyname | student\_age |
| -- | -------------------------- | ------------------- | ------------ |
| 1  | Meimei                     | Han                 | 12           |
| 2  | Lily                       | Jaskson             | 15           |
...
空单元格选项通过三种方式处理Excel/WPS中的空单元格：

1. `空字符串 ""` 或 `"emptyString"`：将空单元格转换为空字符串 `""`
2. `JSON null` 或 `"null"`：将空单元格转换为 `null`
3. `不包含在JSON中` 或 `"exclude"`：从JSON中排除空单元格 。此选项在2DArray格式输出JSON中不可用。
...
布尔格式指定如何转换Excel/WPS中的布尔值：

1. `JSON true/false` 或 `"trueFalse"`：转换为JSON布尔值 (`true` 或 `false`)
2. `字符串 "true"/"false"` 或 `"string"`：转换为字符串 (`"true"` 或 `"false"`)
3. `数字 1/0` 或 `"10"`：转换为数字 (`1` 表示 `TRUE`，`0` 表示 `FALSE`)
...
此功能允许您将Excel/WPS中的日期值转换为特定格式，如ISO 8601或自1900-01-01以来的天数。当您在网页浏览器中加载Excel到JSON时，此功能不可用。如果您想将Excel中的日期值转换为特定格式，可以在Excel/WPS应用程序中侧载Excel到JSON。

日期格式指定如何转换Excel/WPS中的日期值：

1. `自1900-01-01以来的天数`：转换为自1900-01-01以来的天数
2. `字符串，ISO 8601 (YYYY-MM-DDTHH:mm:ssZ)`：转换为ISO 8601格式的字符串

> <mark> 注意：日期格式功能仅在您在Excel/WPS数据表第一行添加 $date$ 作为后缀时才有效。请参考后续示例标题行。 </mark>
...
- 有效文件名：data.json、my\_data.json、export\_2024.json
...
如果您已订阅并希望看到更多功能，请通过电子邮件<he.yang@wtsolutions.cn>联系我们。
